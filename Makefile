# This Makefile is for the WhosThat extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/WhosThat.pm]
#     AUTHOR => [q[rbowen <rbowen@rcbowen.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     EXE_FILES => [q[bin/whosthat], q[bin/whoworkson]]
#     LICENSE => q[apache]
#     NAME => q[WhosThat]
#     PREREQ_PM => { Test::More=>q[0], YAML::Tiny=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/WhosThat.pm]
...
OBJ_EXT = .o
OSNAME = linux
OSVERS = 6.2.8-200.fc37.x86_64
RANLIB = :
SITELIBEXP = /usr/local/share/perl5/5.36
SITEARCHEXP = /usr/local/lib64/perl5/5.36
SO = so
...
MAN3SECTION = 3
INSTALLDIRS = site
INSTALL_BASE = /home/rbowen/perl5
DESTDIR = 
PREFIX = $(INSTALL_BASE)
...
EXE_FILES = bin/whosthat bin/whoworkson

pure_all :: $(INST_SCRIPT)/whosthat $(INST_SCRIPT)/whoworkson
	$(NOECHO) $(NOOP)
...
$(INST_SCRIPT)/whosthat : bin/whosthat $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	...
	$(CP) bin/whosthat $(INST_SCRIPT)/whosthat
	$(FIXIN) $(INST_SCRIPT)/whosthat
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/whosthat

$(INST_SCRIPT)/whoworkson : bin/whoworkson $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/whoworkson
	$(CP) bin/whoworkson $(INST_SCRIPT)/whoworkson
	$(FIXIN) $(INST_SCRIPT)/whoworkson
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/whoworkson
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - '\''rbowen <rbowen@rcbowen.com>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: WhosThat' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '  YAML::Tiny: '\''0'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      "rbowen <rbowen@rcbowen.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "WhosThat",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "YAML::Tiny" : "0"' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="WhosThat" VERSION="0.01">' > WhosThat.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> WhosThat.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>rbowen &lt;rbowen@rcbowen.com&gt;</AUTHOR>' >> WhosThat.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> WhosThat.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::More" />' >> WhosThat.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="YAML::Tiny" />' >> WhosThat.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-thread-multi-5.36" />' >> WhosThat.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> WhosThat.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> WhosThat.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> WhosThat.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/WhosThat.pm' 'blib/lib/WhosThat.pm' 
